routerAdd('POST', '/backend/v1/onedrive_get_token', (e) => {
  try {
    e.response.header().set('Access-Control-Allow-Origin', '*')

    const isCron = e.request.header.get('X-Cron-Secret') === 'cron_internal_call'
    if (!isCron && !e.hasSuperuserAuth() && !e.auth) {
    ...
    globalThis.__onedrive_token_cache = globalThis.__onedrive_token_cache || {
      token: null,
      expiresAt: 0,
    }
    ...
    const now = Date.now()
    if (
      globalThis.__onedrive_token_cache.token &&
      globalThis.__onedrive_token_cache.expiresAt > now + 60000
    ) {
      return e.json(200, {
        token: globalThis.__onedrive_token_cache.token,
        expiresIn: Math.floor((globalThis.__onedrive_token_cache.expiresAt - now) / 1000),
      })
    }
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        $app
          .logger()
          .error('Erro ao obter token do OneDrive', 'err', String(error), 'attempt', attempt)
    ...
    return e.json(500, {
      error: 'Erro de rede, tentando novamente falhou. Erro ao obter token do OneDrive.',
    })
  } catch (globalErr) {
